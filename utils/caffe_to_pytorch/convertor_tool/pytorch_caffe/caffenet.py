...
class CaffeData(nn.Module):
    def __init__(self, layer):
        ...
        caffe.set_mode_cpu()
        if 'include' in layer.keys() and layer['include'] == 'TRAIN':
        ...
        self.register_buffer('imL', torch.zeros(1))
...
class CaffeNet(nn.Module):
    def __init__(self, protofile, width=None, height=None, channels=None, omit_data_layer=False, phase='TRAIN'):
        ...
        self.has_mean = False
        if 'mean_file' in self.net_info['props'].keys():
            self.has_mean = True
    ...
    def forward(self, *inputs): 
        ...
        while i < layer_num:
            layer = layers[i]
            lname = layer['name']
            if 'include' in layer.keys() and 'phase' in layer['include'].keys():
    ...
    def load_weights(self, caffemodel):
        if self.has_mean:
            ...
            if 'input_shape' in self.net_info['props'].keys():
        ...
        while i < layer_num:
            ...
            if ltype in ['Convolution', 'Deconvolution']:
                ...
                bias = True
                if 'bias_term' in convolution_param.keys() and convolution_param['bias_term'] == 'false':
                    bias = False
                
                caffe_weight = np.array(lmap[lname].blobs[0].data)
                # caffe_weight = torch.from_numpy(caffe_weight).view_as(self.models[lname].weight)
                caffe_weight = torch.from_numpy(caffe_weight).view_as(self.models[lname].weight).float()
                self.models[lname].weight.data.copy_(caffe_weight)
                if bias and len(lmap[lname].blobs) > 1:
                    # self.models[lname].bias.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data, dtype=np.float32)))
                ...
                # self.models[lname].running_mean.copy_(torch.from_numpy(np.array(lmap[lname].blobs[0].data, dtype=np.float32) / lmap[lname].blobs[2].data[0]))
                self.models[lname].running_var.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data) / lmap[lname].blobs[2].data[0]))
                # self.models[lname].running_var.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data, dtype=np.float32) / lmap[lname].blobs[2].data[0]))
                i = i + 1
            elif ltype == 'Scale':
                self.models[lname].weight.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[0].data)))
                # self.models[lname].weight.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[0].data, dtype=np.float32)))

                if len(lmap[lname].blobs) > 1:
                    # self.models[lname].bias.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data, dtype=np.float32)))
                    self.models[lname].bias.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data)))
                else:
                    # self.models[lname].bias.data.copy_(torch.from_numpy(np.array([0], dtype=np.float32)))
                    self.models[lname].bias.data.copy_(torch.from_numpy(np.array([0])))
                i = i + 1
                ...
                if type(self.models[lname]) == nn.Sequential:
                    ...
                    # self.models[lname][1].weight.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[0].data, dtype=np.float32)))
                    if len(lmap[lname].blobs) > 1:
                        # self.models[lname][1].bias.data.copy_(torch.from_numpy(np.array(lmap[lname].blobs[1].data, dtype=np.float32)))
    ...
    def create_network(self, net_info, input_width = None, input_height = None, input_channels = None):
        ...
        # data layer is 'imL' - chaning from 'data' to 'iml' - but currenlt imL not in blob_channels but have data. chaning name??
        # blob_channels['data'] = 3
        blob_channels['imL'] = 3
        if input_channels != None:
            # blob_channels['data'] = input_channels
            blob_channels['imL'] = input_channels
        # blob_height['data'] = 1
        blob_height['imL'] = 1
        if input_height != None:
            # blob_height['data'] = input_height
            blob_height['imL'] = input_height
        # blob_width['data'] = 1
        blob_width['imL'] = 1
        if input_width != None:
            # blob_width['data'] = input_width
            blob_width['imL'] = input_width
        if 'input_shape' in props.keys():
            ...
            blob_channels['imL'] = int(props['input_shape']['dim'][1])
            blob_height['imL'] = int(props['input_shape']['dim'][2])
            blob_width['imL'] = int(props['input_shape']['dim'][3])
    
            self.width = int(props['input_shape']['dim'][3])
            self.height = int(props['input_shape']['dim'][2])
        elif 'input_dim' in props.keys():
            # blob_channels['data'] = int(props['input_dim'][1])
            # blob_height['data'] = int(props['input_dim'][2])
            # blob_width['data'] = int(props['input_dim'][3])
            blob_channels['imL'] = int(props['input_dim'][1])
            blob_height['imL'] = int(props['input_dim'][2])
            blob_width['imL'] = int(props['input_dim'][3])
        ...
        while i < layer_num:
            ...
            if ltype in ['Data', 'AnnotatedData']:
                ...
                i = i + 1
                continue
            bname = layer['bottom'] # layer['bottom'] is 'imL' - but it's not in blob_channels??
            # bname = 'data'   
            ...
            if ltype == 'Convolution':
                ...
                stride = int(convolution_param['stride']) if 'stride' in convolution_param.keys() else 1
                pad = int(convolution_param['pad']) if 'pad' in convolution_param.keys() else 0
                group = int(convolution_param['group']) if 'group' in convolution_param.keys() else 1
                dilation = 1
                if 'dilation' in convolution_param.keys():
                ...
                momentum = 0.9
                if 'batch_norm_param' in layer.keys() and 'moving_average_fraction' in layer['batch_norm_param'].keys():
                ...
                inplace = (bname == tname)
                if 'relu_param' in layer.keys() and 'negative_slope' in layer['relu_param'].keys():
                ...
                padding = 0
                if 'pad' in layer['pooling_param'].keys():
                ...
                operation = 'SUM'
                if 'eltwise_param' in layer.keys() and 'operation' in layer['eltwise_param'].keys():
                ...
                axis = 1
                if 'concat_param' in layer.keys() and 'axis' in layer['concat_param'].keys():
                ...
                max_size = -1
                if 'max_size' in layer['prior_box_param'].keys():
                    max_size = float(layer['prior_box_param']['max_size'])
                aspects = []
                if 'aspect_ratio' in layer['prior_box_param'].keys():
                ...
                flip = False
                if 'flip' in layer['prior_box_param'].keys():
                ...
                # models[lname] = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=(kernel_size, kernel_size), stride = (stride,stride), padding=(padding, padding), groups = group, bias=bias_term)
                ...
                axis = 1
                if 'softmax_param' in layer.keys() and 'axis' in layer['softmax_param'].keys():
